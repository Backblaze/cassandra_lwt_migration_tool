...
class CassandraOnOneNode:
    ...
    def check_completion(self, force_baseline_file_usage: bool) -> int:
        self.node_print(f'Checking completion with {options.baseline_directory} as user {options.cassandra_username}.')
        baseline_path = os.path.join(options.baseline_directory, f'{self.node_name}.json')
        updated_baseline_path = os.path.join(options.baseline_directory,
                                             f'{self.UPDATE_FILE_PREFIX}{self.node_name}.json')

        baseline_state = None

        path_to_read = (updated_baseline_path if os.path.exists(updated_baseline_path) else baseline_path)
        ...
        for map_key, row in baseline_state.rows.items():
            if (matched_row := captured_rows.rows.get(map_key, None)) is not None and \
                    matched_row.in_progress_ballot == row.in_progress_ballot:
    ...
    def raise_if_not_connected_to_ip(self):
        ...
        if ip_address(self.node_ip) != ip_address(listen_addr):
            raise CassandraSingleNodeError(f'Not connected to correct node [{self.node_ip}] != [{listen_addr}] ')
    ...
    def visit_token_range(self, token_range: Tuple[cassandra.metadata.Token, cassandra.metadata.Token],
        ...
        for retry in range(self.NUM_RETRIES):
            try:
                ...
                logging.warning('Error fetching the next set of rows... last_row: %s token_range: %s', last_row_key,
