...
OPERATION_MODE = Literal['captureBaseline', 'checkCompletion', 'checkBaselineCompletion', 'checkTargetingNodes']
...
class CassandraLWTMigrationToolOptions(argparse.Namespace):
    ...
    def populate(self):
        ...
        _parser.add_argument('node_ips_file_path', default='.', help='A path to a file containing the cassandra node IPs.', type=pathlib.Path)
        _parser.add_argument('baseline_directory', default='.', help='The directory to store the baseline information to.', type=pathlib.Path)

        _parser.add_argument('--cassandra-username', default=None, help='The username to authenticate to cassandra with.')
        _parser.add_argument('--cassandra-password', default=None, help='The password to authenticate to cassandra with.')
